...
def test_revoke_by_issuer():
    ...
    verify_ocsp(cert_file_pem, "/tmp/intermediate-cert-rsa.pem", ee_ocsp_url, "revoked")
    verify_akamai_purge()
...
def test_ocsp():
    ...
    verify_ocsp(cert_file_pem, "/tmp/intermediate-cert-rsa.pem", ee_ocsp_url, "good")
...
def ocsp_exp_unauth_setup():
    ...
    # would fail).
    # check_ocsp_basic_oid(cert_file_pem, "/tmp/intermediate-cert-rsa.pem", "http://localhost:4002")
    check_ocsp_basic_oid(cert_file_pem, "/tmp/intermediate-cert-rsa.pem", "http://localhost:4002")
...
def test_ocsp_exp_unauth():
    ...
    while True:
        try:
            verify_ocsp(expired_cert_name, "/tmp/intermediate-cert-rsa.pem", "http://localhost:4002", "XXX")
...
def test_admin_revoker_batched():
    ...
    ee_ocsp_url = "http://localhost:4002"
    for cert in certs:
        verify_ocsp(cert, "/tmp/intermediate-cert-rsa.pem", ee_ocsp_url, "revoked")
